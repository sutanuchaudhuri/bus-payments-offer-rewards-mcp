...
class OfferStatistics(BaseModel):
    ...
    total_activations: Optional[int] = Field(None, description="Total number of customer activations")
    active_activations: Optional[int] = Field(None, description="Number of currently active activations")

class Offer(BaseModel):
    """Offer entity model"""
    id: Optional[int] = Field(None, description="Unique offer identifier")
    ...
    min_transaction_amount: Optional[float] = Field(None, description="Minimum transaction amount to qualify")
    reward_points: Optional[int] = Field(0, description="Reward points for this offer")
    max_usage_per_customer: Optional[int] = Field(1, description="Maximum times a customer can use this offer")
    ...
    is_active: Optional[bool] = Field(True, description="Whether the offer is active")
    terms_and_conditions: Optional[str] = Field(None, description="Terms and conditions")
    created_at: Optional[datetime] = Field(None, description="Creation timestamp")
    # Customer-specific fields
    customer_activated: Optional[bool] = Field(None, description="Only present when customer_id is provided")
    activation_date: Optional[datetime] = Field(None, description="Only present when customer_id is provided and offer is activated")
    used_count: Optional[int] = Field(None, description="Only present when customer_id is provided and offer is activated")
    # Statistics (for detailed view)
    statistics: Optional[OfferStatistics] = Field(None, description="Offer statistics")

class OfferCreate(BaseModel):
    """Create a new offer with the specified details"""
    title: str = Field(..., description="Offer title")
    description: Optional[str] = Field(None, description="Offer description")
    category: OfferCategory = Field(..., description="Offer category")
    merchant_id: Optional[int] = Field(None, description="Associated merchant ID")
    ...
    min_transaction_amount: Optional[float] = Field(None, ge=0, description="Minimum transaction amount to qualify")
    reward_points: Optional[int] = Field(0, ge=0, description="Reward points for this offer")
    max_usage_per_customer: Optional[int] = Field(1, description="Maximum times a customer can use this offer")
...
class OfferUpdate(BaseModel):
    ...
    max_discount_amount: Optional[float] = Field(None, ge=0, description="Maximum discount amount")
    min_transaction_amount: Optional[float] = Field(None, ge=0, description="Minimum transaction amount to qualify")
    reward_points: Optional[int] = Field(None, ge=0, description="Reward points for this offer")
    max_usage_per_customer: Optional[int] = Field(None, description="Maximum times a customer can use this offer")
    start_date: Optional[datetime] = Field(None, description="Offer start date")
    expiry_date: Optional[datetime] = Field(None, description="Offer expiry date")
    terms_and_conditions: Optional[str] = Field(None, description="Terms and conditions")
    is_active: Optional[bool] = Field(None, description="Whether the offer is active")

class OfferActivationRequest(BaseModel):
    """Activate an offer for a specific customer"""
    customer_id: int = Field(..., description="Customer ID to activate the offer for")
